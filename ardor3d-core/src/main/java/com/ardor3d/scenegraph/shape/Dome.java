    ...
    private void setGeometryData(final boolean outsideView, final Vector3 center) {
        ...
        for (int iY = 0; iY < (_planes - 1); iY++) {
            ...
            for (int iR = 0; iR < _radialSamples; iR++) {
                ...
                if (outsideView) {
                    _meshData.getNormalBuffer().put((float) kNormal.getX()).put((float) kNormal.getY()).put(
                            (float) kNormal.getZ());
                } else {
                    _meshData.getNormalBuffer().put((float) -kNormal.getX()).put((float) -kNormal.getY()).put(
                            (float) -kNormal.getZ());
        ...
        // pole
        _meshData.getVertexBuffer().put((float) center.getX()).put((float) (center.getY() + _radius)).put(
                (float) center.getZ());
