...
public final class FastMath {
    ...
    public final static double asin(final double x) {
        return x * (Math.abs(x) * (Math.abs(x) * _asin_a + _asin_b) + _asin_c) + Math.signum(x)
    ...
    public final static double atan(final double x) {
        return (Math.abs(x) < 1) ? x / (1 + _atan_a * x * x) : Math.signum(x) * MathUtils.HALF_PI - x
                / (x * x + _atan_a);
    }
    ...
    /*
     * final static double _asin(final double x) { return atan(x / Math.sqrt(1 + x x)); }
     * 
     * final static double _acos(final double x) { return PI_HALVES - _asin(x); }
     */
}
