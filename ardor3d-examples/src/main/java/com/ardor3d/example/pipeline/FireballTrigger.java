        ...
        GameTaskQueueManager.getManager(example.getCanvas().getCanvasRenderer().getRenderContext()).update(
                new Callable<Void>() {
                    ...
                    public Void call() throws Exception {
                        addParticles(applyToPose, manager);
                        return null;
                    }
                });
    }

    private void addParticles(final SkeletonPose applyToPose, final AnimationManager manager) {
        // find location of fist
        final Transform loc = applyToPose.getGlobalJointTransforms()[15];
