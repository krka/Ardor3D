...
public class TextureProjectionExample extends ExampleBase {
    ...
    @Inject
    public TextureProjectionExample(final LogicalLayer logicalLayer, final FrameHandler frameWork) {
        super(logicalLayer, frameWork);
    }

    @Override
    protected void updateExample(final ReadOnlyTimer timer) {
        projectLoc.set(MathUtils.sin(timer.getTimeInSeconds() - MathUtils.PI)
                * (MathUtils.sin(timer.getTimeInSeconds() * 1.1) * 15.0 + 20.0), MathUtils.sin(timer.getTimeInSeconds()
                * 0.7 - MathUtils.PI) * 10.0 + 30.0, MathUtils.cos(timer.getTimeInSeconds() * 0.4 - MathUtils.PI)
                * (MathUtils.sin(timer.getTimeInSeconds()) * 15.0 + 20.0));
        projectTarget.set(MathUtils.sin(timer.getTimeInSeconds() * 0.4)
                * (MathUtils.sin(timer.getTimeInSeconds() * 0.7) * 10.0 + 10.0), 0.0, MathUtils.cos(timer
                .getTimeInSeconds() * 0.3)
