...
public class TerrainGridCache implements TerrainCache, Runnable {
    ...
    private final static ThreadPoolExecutor tileThreadService;
    private static int nrThreads = 20;
    static {
        tileThreadService = new ThreadPoolExecutor(TerrainGridCache.nrThreads, TerrainGridCache.nrThreads, 0L,
                TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>(), new ThreadFactoryBuilder()
                        .setThreadFactory(Executors.defaultThreadFactory()).setDaemon(true).setNameFormat(
                                "TerrainTileThread-%s").setPriority(Thread.MIN_PRIORITY).build());
    }
    ...
    public void setCurrentPosition(final int x, final int y) {
        ...
        TerrainGridCache.tileThreadService.purge();
    ...
    private void threadedUpdateTiles() {
        ...
        while (tileIterator.hasNext()) {
            ...
            if (validTiles == null || validTiles.contains(data.sourceTile)) {
                ...
                final Future<Boolean> future = TerrainGridCache.tileThreadService.submit(data);
                data.future = future;
            }
            tileIterator.remove();
        }
    ...
    public float getSubHeight(final float x, final float z) {
        ...
        if (!tileData.isValid) {
            ...
            return (float) MathUtils.lerp(intOnZ, MathUtils.lerp(intOnX, topLeft, topRight), MathUtils.lerp(intOnX,
    ...
    public static class TileLoadingData implements Callable<Boolean> {
        ...
        public Boolean call() throws Exception {
            ...
            final Region region = new Region(clipmapLevel, sourceTile.getX() * tileSize * vertexDistance, sourceTile
                    .getY()
