...
public class TextureGridCache implements TextureCache, Runnable {
    ...
    static {
        tileThreadService = new ThreadPoolExecutor(TextureGridCache.nrThreads, TextureGridCache.nrThreads, 0L,
                TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>(), new ThreadFactoryBuilder()
                        .setThreadFactory(Executors.defaultThreadFactory()).setDaemon(true).setNameFormat(
                                "TextureTileThread-%s").setPriority(Thread.MIN_PRIORITY).build());
    }
    ...
    public void setCurrentPosition(final int x, final int y) {
        ...
        TextureGridCache.tileThreadService.purge();
    ...
    private void threadedUpdateTiles() {
        ...
        while (tileIterator.hasNext()) {
            ...
            if (validTiles == null || validTiles.contains(data.sourceTile)) {
                ...
                final Future<Boolean> future = TextureGridCache.tileThreadService.submit(data);
                data.future = future;
            }
            tileIterator.remove();
        }
