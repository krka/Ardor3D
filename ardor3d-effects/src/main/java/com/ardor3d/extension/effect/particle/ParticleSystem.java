...
public abstract class ParticleSystem extends Node {
    ...
    public void initParticleLocation(final int index) {
        ...
        if (_particleType == ParticleType.GeomMesh) {
            ...
            for (int x = 0; x < 3; x++) {
                ...
                BufferUtils.populateFromBuffer(vertices[x], mData.getVertexBuffer(), mData.getIndexBuffer().get(
                        mData.getVertexIndex(index, x, 0)));
            }
    ...
    protected void rotateVectorSpeed(final Vector3 pSpeed) {
        ...
        pSpeed
                .setX(-1
                        * ((_rotMatrix.getValue(0, 0) * x) + (_rotMatrix.getValue(1, 0) * y) + (_rotMatrix.getValue(2,
                                0) * z)));

        pSpeed.setY((getRotMatrix().getValue(0, 1) * x) + (_rotMatrix.getValue(1, 1) * y)
                + (_rotMatrix.getValue(2, 1) * z));

        pSpeed
                .setZ(-1
                        * ((_rotMatrix.getValue(0, 2) * x) + (_rotMatrix.getValue(1, 2) * y) + (_rotMatrix.getValue(2,
                                2) * z)));
    }
    ...
    public void updateGeometricState(final double time, final boolean initiator) {
        ...
        if (_particlesInWorldCoords) {
            _emitterTransform.setRotation(getWorldRotation());
            _emitterTransform.setTranslation(getWorldTranslation());
            _emitterTransform.setScale(getWorldScale());
